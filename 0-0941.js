    ...
    if (!directionsInitialized) {
        ...
        directions.on('origin', (event) => {
            ...
            if (originCoordinates) {
                const originInput = document.querySelector('.mapbox-directions-origin input');
                if (originInput) {
                    originInput.placeholder = sidebarheader || originCoordinates || 'Choose a starting place';
                    originInput.value = sidebarheader || originProperties.title || originInput.value || '';
        ...
        directions.on('destination', (event) => {
            ...
            if (destinationCoordinates) {
                const destinationInput = document.querySelector('.mapbox-directions-destination input');
                if (destinationInput) {
                    destinationInput.placeholder = sidebarheader || destinationProperties.title || 'Choose destination';
                    destinationInput.value = sidebarheader || destinationCoordinates || destinationProperties.title || destinationInput.value || '';
