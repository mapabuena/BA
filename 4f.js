...
function updateInfoWindowContent() {
 ...
 const visibleMarkers = markers.filter(({ marker }) => {
    ...
    const isInBounds = map.getBounds().contains(position);
    const isVisible = marker.getElement().style.display !== 'none';
    console.log(`Marker at ${position}: isInBounds=${isInBounds}, isVisible=${isVisible}`);
    return isInBounds && isVisible;
});

    console.log(`Visible markers count: ${visibleMarkers.length}`);
    visibleMarkers.sort((a, b) => calculateDistance(center.lat, center.lng, a.data.lat, a.data.lng) - calculateDistance(center.lat, center.lng, b.data.lat, b.data.lng));

const infoWindow = document.getElementById('info-window');
infoWindow.innerHTML = ''; // Make sure this is effectively clearing the content
