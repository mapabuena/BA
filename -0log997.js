    ...
    if (directionsStepsElement) {
        ...
        if (route.legs && route.legs.length > 0) {
            route.legs.forEach((leg, index) => {
                console.log(`Leg ${index}:`, leg.steps); // Log each leg's steps
                if (leg.steps && leg.steps.length > 0) {
                    ...
                    console.warn(`No steps found in leg ${index}.`);
            ...
            console.warn("No legs or steps found in the route.");
            const noStepsElement = document.createElement('div');
            noStepsElement.className = 'mapbox-directions-no-steps';
            noStepsElement.innerText = 'No step-by-step directions available.';
            directionsStepsElement.appendChild(noStepsElement);
...
// Modified checkAndRetrieveDirections function
function checkAndRetrieveDirections() {
    ...
    if (originCoordinates && destinationCoordinates) {
        ...
        const url = `https://api.mapbox.com/directions/v5/mapbox/${profile}/${originCoords};${destinationCoords}?access_token=${mapboxgl.accessToken}&alternatives=true&geometries=geojson&steps=true`;

        fetch(url)
            .then(response => response.json())
            .then(data => {
                console.log("Routes received:", data.routes);
                handleRouteData(data); // Handle the route data
            })
...
function addRouteToMap(route, isBestRoute) {
    ...
    const routeSourceId = isBestRoute ? 'best-route' : `alternative-route-${route.geometry.coordinates[0].join('-')}`;
    ...
    if (map.getSource(routeSourceId)) {
        ...
        map.addLayer({
            ...
            paint: {
                'line-color': isBestRoute ? '#3b9ddd' : '#cccccc',
                'line-width': isBestRoute ? 5 : 3,
                'line-opacity': isBestRoute ? 0.75 : 0.5
            }
        });
    }
}
...
function clearRouteFromMap() {
    if (map.getSource('route')) {
        console.log("Removing existing route source from the map.");
...
function clearAllRoutes() {
    ...
    if (layers) {
        for (let i = layers.length - 1; i >= 0; i--) {
            if (layers[i].id.startsWith('route-layer') || layers[i].id.startsWith('alternative-route')) {
                map.removeLayer(layers[i].id);
                map.removeSource(layers[i].id.replace('layer', 'source'));
            }
        }
    }
}
// Function to handle route data from the API and add it to the map
function handleRouteData(data) {
    if (data.routes && data.routes.length > 0) {
        clearAllRoutes(); // Clear previous routes
        const bestRoute = data.routes[0];
        addRouteToMap(bestRoute, true);
        if (data.routes.length > 1) {
            const alternativeRoutes = data.routes.slice(1);
            alternativeRoutes.forEach(route => addRouteToMap(route, false));
        }
        updateDirectionsSteps(bestRoute);
    } else {
