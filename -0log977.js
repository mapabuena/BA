...
function addRoutePopup(route, index, profile) {
    ...
    let popupPosition;
    if (index === 0) {
        popupPosition = Math.floor(coordinates.length * 0.3); // 30% along the route
    } else if (index === 1) {
        popupPosition = Math.floor(coordinates.length * 0.45); // 45% along the route
    } else {
        popupPosition = Math.floor(coordinates.length * 0.6); // 60% along the route
    }
    ...
    const popupContent = `
        <img src="${modeIcon}" alt="Mode" class="route-icon">
        <div class="route-info">
            <p class="route-time">${formattedTravelTime}</p>
            <p class="route-distance">${formattedDistance}</p>
        </div>
    `;
