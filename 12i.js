...
function updateInfoWindowContent() {
    ...
    const infoWindow = document.getElementById('infowindowbar'); // Updated ID
    infoWindow.innerHTML = '';  // Clear current content if needed

    // Logic to update the content
    const visibleMarkers = markers.filter(({ marker }) => {
        return bounds.contains(marker.getLngLat()) && marker.getElement().style.display !== 'none';
    });
    ...
    if (visibleMarkers.length === 0) {
        infoWindow.innerHTML = 'No visible markers within bounds.';
    }
}
