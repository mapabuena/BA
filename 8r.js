...
function applyFilters() {
    ...
    markers.forEach(({ marker, data }) => {
        ...
        // Check if any date range within the marker data matches the filter criteria
        const isVisibleByDate = data.dateRanges.some(range => {
            const rangeStart = new Date(range.start);
            const rangeEnd = new Date(range.end);
            return rangeStart >= startDateTime && rangeEnd <= endDateTime;
        });

        // Update marker display based on visibility
        marker.getElement().style.display = (isVisibleByCategory && isVisibleByDate) ? '' : 'none';
    });

    updateInfoWindowContent(); // Refresh information display based on filter results
}
