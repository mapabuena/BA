...
initializeDatePicker(); // Initialize date picker
 ...
 // Function to setup Pikaday date picker with time selection
    function initializeDatePicker() {
        const startDateInput = document.getElementById('startDate');
        const endDateInput = document.getElementById('endDate');

        if (!startDateInput || !endDateInput) {
            console.error("Date picker inputs are missing.");
            return;
        }

        // Extend Pikaday to include a time selector
        Pikaday.prototype.draw = (function() {
            var originalDraw = Pikaday.prototype.draw;
            return function() {
                originalDraw.call(this);
                var datepicker = this.el;
                if (!datepicker.querySelector('.time-container')) {
                    var timeContainer = document.createElement('div');
                    timeContainer.className = 'time-container';

                    var hourSelect = document.createElement('select');
                    hourSelect.className = 'pika-select-hour';
                    for (var i = 0; i < 24; i++) {
                    ...
                    minuteSelect.className = 'pika-select-minute';
                    for (var j = 0; j < 60; j += 5) {
                        var option = document.createElement('option');
                        option.value = j;
                        option.textContent = ('0' + j).slice(-2);
                        minuteSelect.appendChild(option);
                    }

                    timeContainer.appendChild(hourSelect);
                    timeContainer.appendChild(minuteSelect);
                    datepicker.appendChild(timeContainer);

                    hourSelect.addEventListener('change', () => updateDateTime(this, hourSelect, minuteSelect));
        ...
        function updateDateTime(pikadayInstance, hourSelect, minuteSelect) {
            ...
            var date = pikadayInstance.getDate() || new Date();
            date.setHours(selectedHour);
            date.setMinutes(selectedMinute);
            pikadayInstance.setDate(date);
            pikadayInstance._o.field.value = pikadayInstance.toString();
        }
        ...
        var startPicker = new Pikaday({
            ...
            showTime: true,
            onSelect: function() {
                endDateInput.min = startPicker.toString();
                checkAndApplyFilter();
            }
        });
        ...
        var endPicker = new Pikaday({
            ...
            showTime: true,
            onSelect: function() {
                startDateInput.max = endPicker.toString();
                checkAndApplyFilter();
            }
        });
        ...
        function applyDateFilter() {
            ...
            activeFilters.category2 = generateDateRange(startDateInput.value, endDateInput.value);
