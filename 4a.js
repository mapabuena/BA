...
function updateInfoWindowContent() {
    ...
    const visibleMarkers = markers.filter(({ marker }) => {
        ...
        const isVisible = bounds.contains(position) && marker.getElement().style.display !== 'none';
        console.log(`Marker at ${position}: visible=${isVisible}`);
        return isVisible;
    });

    console.log(`Visible markers: ${visibleMarkers.length}`);
    if (visibleMarkers.length === 0) {
        console.log("No visible markers within bounds.");
        return;
    }

    visibleMarkers.sort((a, b) => calculateDistance(center.lat, center.lng, a.data.lat, a.data.lng) -
                                  calculateDistance(center.lat, center.lng, b.data.lat, b.data.lng));
