...
directions.on('route', (event) => {
    ...
    if (routes && routes.length > 0) {
        ...
        // Create a custom directions content
        const directionsContent = createDirectionsContent(route);
        customDirectionsDiv.innerHTML = ''; // Clear existing content
        customDirectionsDiv.appendChild(directionsContent);
    }
});
function createDirectionsContent(route) {
    const container = document.createElement('div');
    container.className = 'custom-directions-content';

    // Add summary
    const summary = document.createElement('div');
    summary.className = 'custom-directions-summary';
    summary.innerHTML = `
        <strong>Distance:</strong> ${(route.distance / 1000).toFixed(2)} km<br>
        <strong>Duration:</strong> ${Math.round(route.duration / 60)} mins
    `;
    container.appendChild(summary);

    // Add steps
    const stepsContainer = document.createElement('div');
    stepsContainer.className = 'custom-directions-steps';
    route.legs[0].steps.forEach((step, index) => {
        const stepDiv = document.createElement('div');
        stepDiv.className = 'custom-directions-step';
        stepDiv.innerHTML = `${index + 1}. ${step.maneuver.instruction}`;
