...
function createCustomMarker(coords, svgUrl, draggable = false) {
    ...
    return new mapboxgl.Marker(el, { draggable }).setLngLat(coords);
}


// Create or update the origin marker
function updateOriginMarker(coords) {
    console.log('updateOriginMarker called with coords:', coords);
    const originSvgUrl = 'https://raw.githubusercontent.com/mapabuena/BA/main/TransparentMapIconRed.svg';
    if (customOriginMarker) {
        customOriginMarker.setLngLat(coords).addTo(map);
    } else {
        customOriginMarker = createCustomMarker(coords, originSvgUrl, true).addTo(map);
        customOriginMarker.on('dragend', () => {
...
function updateDestinationMarker(coords) {
    console.log('updateDestinationMarker called with coords:', coords);
    const destinationSvgUrl = 'https://raw.githubusercontent.com/mapabuena/BA/main/TransparentMapIcon.svg';
    if (customDestinationMarker) {
        customDestinationMarker.setLngLat(coords).addTo(map);
    } else {
        customDestinationMarker = createCustomMarker(coords, destinationSvgUrl, true).addTo(map);
        customDestinationMarker.on('dragend', () => {
...
function initializeDirectionsControl() {
    if (!directions) {
        directions = new MapboxDirections({
            ...
            fitBounds: false, // Prevent automatic fitting to bounds
            geometries: 'polyline6' // Use polyline6 for more accurate routes
        });
        ...
        checkAndRetrieveDirections();
            ...
            directions.on('route', (event) => {
                ...
                if (event.route) {
                    ...
                    console.warn("No route found in the event.");
                }
...
function updateDirectionsSteps(route) {
    ...
    if (directionsStepsElement) {
        directionsStepsElement.innerHTML = ''; // Clear existing steps
            ...
            route.legs.forEach((leg) => {
                if (leg.steps && leg.steps.length > 0) {
                    leg.steps.forEach(step => {
                        ...
                        stepElement.className = 'mapbox-directions-step';
                        stepElement.innerHTML = `
                            <div class="mapbox-directions-icon mapbox-directions-icon-${step.maneuver.type}"></div>
                            <div class="mapbox-directions-instruction">
                                <div class="mapbox-directions-step-maneuver">
                                    ${step.maneuver.instruction}
                                </div>
                                <div class="mapbox-directions-step-distance">${step.distance} meters</div>
                            </div>
                        `;
                        directionsStepsElement.appendChild(stepElement);
                    });
                } else {
                    const noStepsElement = document.createElement('div');
                    noStepsElement.className = 'mapbox-directions-no-steps';
                    noStepsElement.innerText = 'No step-by-step directions available.';
                    directionsStepsElement.appendChild(noStepsElement);
                }
            });
        } else {
            const noStepsElement = document.createElement('div');
        ...
        console.error("Directions steps element not found.");
    }
}
...
function checkAndRetrieveDirections() {
    ...
    if (originCoordinates && destinationCoordinates) {
        ...
        const url = `https://api.mapbox.com/directions/v5/${currentProfile}/${originCoords};${destinationCoords}?access_token=${mapboxgl.accessToken}&alternatives=true&geometries=polyline6&steps=true`;
...
function addRoutePopup(route, index, profile) {
    ...
    switch (profile) {
        case 'mapbox/driving':
            ...
            popupSize = { width: '60px', height: '16px' };
        ...
        case 'mapbox/walking':
            ...
            popupSize = { width: '65px', height: '14px' };
        ...
        case 'mapbox/cycling':
            ...
            popupSize = { width: '66px', height: '12px' };
        ...
        default:
            ...
            popupSize = { width: '70px', height: '16px' };
            iconPaddingBottom = '32px';
    }
    const popupContent = `
...
function adjustZoomIfNeeded(origin, destination) {
    ...
    const originInView = bounds.contains(origin);
    const destinationInView = bounds.contains(destination);

    if (!originInView || !destinationInView) {
        ...
        // Fit the map to the new bounds without changing the center
        map.fitBounds(newBounds, {
            padding: 50,
            maxZoom: map.getZoom(),
            center: currentCenter,
            linear: true,
            duration: 0 // Set to 0 for instant zoom, or adjust as needed
        });
    }
}
...
function clearRouteFromMap() {
    // Iterate through all potential route layers and sources to remove them
    for (let i = 0; i < 10; i++) {
    ...
    console.log("Cleared all route layers and sources.");
}
...
function clearAllPopups() {
    if (window.routePopups) {
        window.routePopups.forEach(popup => {
...
function setOriginOnClick(e) {
    ...
    directions.removeRoutes(); // Clear previous routes
...
function changeProfile(profile) {
    console.log('changeProfile called with profile:', profile);
    if (!directions) {
        console.error("Directions control is not initialized.");
        return;
    }
    ...
    console.log('changeProfile function completed');
}
