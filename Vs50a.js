...
function getActiveMarkers() {
    const activeMarkersData = markers.filter(marker => marker.getMap() !== null)
                                      .map(marker => {
                                          // Assuming each marker has an `infowindowContent` property with the necessary data
                                          return marker.infowindowContent;
                                      });

    // Update the sidebar with the data from active markers
    const sidebar = document.getElementById('sidebar');
    sidebar.innerHTML = ''; // Clear existing content
    activeMarkersData.forEach(data => {
        const entry = document.createElement('div');
        entry.innerHTML = `<h3>${data.popup_header}</h3>
                           <img src="${data.popupimage_url}" alt="Image" style="width:100%;">
                           <p>${data.description}</p>`;
        sidebar.appendChild(entry);
    });
}

function attachFilterButtonEventListeners() {
    const buttons = document.querySelectorAll('.button-container button');
    buttons.forEach(button => {
        button.addEventListener('click', function() {
            // Apply filters based on the button clicked.
            // This pseudocode assumes a function `applyFilters()` exists and should be implemented
            // to reflect how filters are applied based on button clicks.
            applyFilters(button.dataset);

            // After filters have been applied and markers updated, refresh the sidebar.
            getActiveMarkers();
        });
    });
}
