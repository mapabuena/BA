...
function applyFilters() {
    ...
    markers.forEach(({ marker, data }) => {
        ...
        const isVisibleByDateRange = data.dateRanges && data.dateRanges.some(range => {
            const rangeStart = new Date(range.start);
            const rangeEnd = new Date(range.end);
        ...
        console.log(`Date Visibility for ${data.address}: ${isVisibleByDateRange || isVisibleByRecurring}`);

        // Update marker display based on combined visibility results
        marker.getElement().style.display = (isVisibleByCategory && (isVisibleByDateRange || isVisibleByRecurring)) ? '' : 'none';
    });
...
function addMarkerToMap(data) {
    // Ensure lat and lng are valid numbers before creating a marker
    const lat = parseFloat(data.latitude);
    const lng = parseFloat(data.longitude);
