...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setProfile(profile) {
        ...
        // Clear previous popups and routes
        clearAllPopups();
        if (directions) {
            directions.removeRoutes();
        }

        // Reinitialize the directions control with the new profile
        ...
        // Manually set the input fields' values
        setDirectionsInputFields(currentOrigin.properties.title || '', currentDestination.properties.title || '');
        ...
        // Ensure routes are drawn when switching profiles
        checkAndDrawRoutes(profile);
    }
...
function initializeDirectionsControl(profile) {
    ...
    const currentOrigin = directions ? directions.getOrigin() : null;
    const currentDestination = directions ? directions.getDestination() : null;
    ...
    if (directionsControlElement && directionsControlElement.innerHTML.trim() === '') {
        ...
        directions.on('route', (event) => {
            ...
            if (routes && routes.length > 0) {
                ...
                checkAndDrawRoutes(profile); // Ensure routes are redrawn if not present
            }
        });

        if (currentOrigin && currentOrigin.geometry) {
            directions.setOrigin(currentOrigin.geometry.coordinates);
            setDirectionsInputFields(currentOrigin.properties.title || '', '');
        }
        if (currentDestination && currentDestination.geometry) {
            directions.setDestination(currentDestination.geometry.coordinates);
            setDirectionsInputFields('', currentDestination.properties.title || '');
        }
    } else {
        console.error("Element with ID 'directions-control' not found or already contains directions.");
    }
...
function checkAndDrawRoutes(profile) {
    const currentOrigin = directions.getOrigin();
    const currentDestination = directions.getDestination();
