...
function updateInfoWindowContent() {
    ...
    const visibleMarkers = markers.filter(({ marker }) => {
        ...
        const isVisible = bounds.contains(pos) && marker.getElement().style.display !== 'none';
        return isVisible;
    });
    ...
    infoWindow.innerHTML = ''; // Always clear previous content
    visibleMarkers.forEach(({ data }) => {
        const item = document.createElement('div');
        item.className = 'info-item';
        item.innerHTML = `<h4>${data.name}</h4><p>Visible and within bounds</p>`;
