...
function updateDirectionsSteps(route) {
    const directionsStepsElement = document.querySelector('.mapbox-directions-step-list');
...
function checkAndRetrieveDirections() {
    ...
    if (originCoordinates && destinationCoordinates) {
        ...
        const url = `https://api.mapbox.com/directions/v5/mapbox/${profile}/${originCoords};${destinationCoords}?access_token=${mapboxgl.accessToken}&geometries=geojson&alternatives=true`;
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.routes && data.routes.length > 0) {
                    ...
                    addRouteToMap(data.routes[0]);  // Add the best route to the map
                } else {
...
function addRouteToMap(route) {
    ...
    if (map.getSource('route')) {
        ...
        map.addLayer({
            ...
            paint: {
                'line-color': 'darkred', // Change to dark red color
                'line-width': 8, // Increase line width
                'line-opacity': 0.75
            }
        });
    }
