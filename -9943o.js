...
function removeDirectionEventListeners() {
    if (directions) {
...
function deactivateClickHandlers() {
    ...
    console.log('Click handlers deactivated');
}

function checkAndLockSettings() {
    if (originSet && destinationSet) {
        deactivateClickHandlers();
...
function initializeDirectionsControl() {
    if (!directions) {
        ...
        if (directionsControlElement) {
            ...
            addDirectionEventListeners(); // Add event listeners
...
function setOriginOnClick(e) {
    ...
    handlingDirectionEvents = true; // Start handling events manually
    ...
    if (selectedMarker) {
        ...
        geocodeAddress(address, function (coords) {
            if (coords) {
                try {
                    removeDirectionEventListeners(); // Temporarily remove event listeners
                    directions.setOrigin(coords); // Set the origin using coordinates
                    addDirectionEventListeners(); // Re-add event listeners
                    ...
                    // Manually handle the origin event
                    handlingDirectionEvents = false; // Stop handling events manually
                } catch (error) {
            ...
            geocodeAddress(address, function (coords) {
                if (coords) {
                    try {
                        ...
                        // Manually handle the destination event
                        handlingDirectionEvents = false; // Stop handling events manually
                    } catch (error) {
